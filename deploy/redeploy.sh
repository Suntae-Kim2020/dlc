#!/usr/bin/env bash
# 이 서버(dl.ailibrary.kr)에 GitHub main 을 반영한다.
#
#   ./deploy/redeploy.sh
#
# 저장소 루트의 deploy.sh 는 헤츠너용(nginx + pm2)이다. 이쪽은 Caddy + systemd
# 구성이라 재시작 방법이 다르다. 전환이 끝나고 헤츠너를 정리하면 그때 하나로
# 합친다.
#
# 서버에서 소스를 수정하지 않는다는 규칙은 여기서도 같다. 자세한 이유는
# README 의 배포 절을 참고.
set -euo pipefail
